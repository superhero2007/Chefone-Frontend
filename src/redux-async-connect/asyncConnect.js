...
export function loadSuccess(key: string, data: any) {
  return {
    type: LOAD_SUCCESS,
    key,
    data,
            ...
            promise: options => {
              ...
              if (promiseOrResult !== undefined) {
                if (promiseOrResult.then instanceof Function) {
                  ...
                  promiseOrResult
                    .then(data => dispatch(loadSuccess(item.key, data)))
...
export function asyncConnect(asyncItems: any) {
  return (Component: any) => {
    Component.reduxAsyncConnect = wrapWithDispatch(asyncItems);

    return connect(state => {
      return asyncItems.reduce(
        (result, item) =>
          item.key
