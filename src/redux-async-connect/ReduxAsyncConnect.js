...
function eachComponents(components, iterator) {
  for (
    let i = 0, l = components.length;
    i < l;
    i++ // eslint-disable-line id-length
  ) {
...
function loadAsyncConnect({ components, filter = () => true, ...rest }) {
  ...
  const promise = Promise.all(
    filterAndFlattenComponents(components).map(Component => {
      ...
      return Promise.all(
        asyncItems.reduce((itemsResults, item) => {
        ...
        const res = asyncItems.reduce(
          (result, item, i) => ({ ...result, ...results[i] }),
          {},
        );
        return res;
      });
...
class ReduxAsyncConnect extends React.Component<*, *> {
  ...
  constructor(props, context) {
    ...
    this.state = {
      propsToShow: __SERVER__ ? (this.isLoaded() ? props : null) : props,
    };
  }
  ...
  componentDidMount() {
    ...
    if (
      !dataLoaded // we dont need it if we already made it on server-side
    ) {
